...
# importing 2024 data
cfb_data_24 <- cfbfastR::load_cfb_pbp(2023:2024) %>%
  dplyr::filter((season == 2023 & week > 5) | season == 2024)

cfb_teams_24 <- cfb_data_24 %>%
  ...
  dplyr::mutate(
    ...
    avg_difference = avg_home_grade - avg_away_grade,
  ) %>%
  ungroup() %>%
  drop_na()

cfb_preds <- cfb_teams_24 %>%
  dplyr::select(
...
cfb_points_pred <- cfb_teams_24 %>%
  ...
  reframe(
    home_team = first(home_team),  # Retain the home_team
    away_team = first(away_team),  # Retain the away_team
...
cfb_rank <- cfb_teams_24 %>%
  ...
  summarise(
    ###
    home_team_grade = sum(home_team_grade, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  drop_na()
...
# team data collection function
get_cfb_team_data <- function(df, team_input, season = c(2023, 2024),
  ...
  team_data <- df %>%
    filter(season %in% season & (home_team == team_input | away_team == team_input))
...
predict_cfb_winner <- function(df, home_team, away_team, season = c(2023,2024), predictor, calibrate = TRUE) {
  home_values <- get_cfb_team_data(df, home_team, season)
  away_values <- get_cfb_team_data(df, away_team, season)
  ...
  logistic_home_pred <- predict(logistic_model_home_cfb, newdata = game_data, type = "response")
  nb_home_pred <- predict(naive_bayes_model_home_cfb, newdata = game_data, type = "raw")[, 2]
  svm_home_pred <- attr(predict(svm_model_home_cfb, newdata = game_data, probability = TRUE), "probabilities")[, 2]
  prob_mean = (logistic_home_pred + nb_home_pred + svm_home_pred) / 3
  
  combined_preds <- data.frame(logistic = logistic_home_pred, nb = nb_home_pred, svm = svm_home_pred, mean_prob = prob_mean)
  
  final_prob_xgb_cfb <- predict(predictor, newdata = as.matrix(combined_preds))
  
  final_prob_tree <- final_prob_xgb_cfb
  
  # platt calibration
  if (calibrate) {
    ensemble_proba <- predict(platt_model_cfb, newdata = data.frame(final_prob_xgb_cfb = final_prob_tree), type = "response")
  } else {
    ensemble_proba <- final_prob_xgb_cfb
  }
...
# getting team data for points projections
get_cfb_team_data_points <- function(df, team_input, season = c(2023, 2024), columns = c("home_team_grade", "away_team_grade", "home_offense", "away_offense", "avg_home_grade", "avg_away_grade")) {
  team_data <- df %>%
    filter(season %in% season & (home_team == team_input | away_team == team_input))
...
# projecting team points scored
predict_cfb_scores <- function(df, home_team, away_team, season = c(2023, 2024), home_model, away_model) {
  home_team_data <- get_cfb_team_data_points(df, home_team, season)
  away_team_data <- get_cfb_team_data_points(df, away_team, season)
  
  home_data <- as.data.frame(t(home_team_data))
  away_data <- as.data.frame(t(away_team_data))
...
home_team_input <- "BYU"
away_team_input <- "Kansas State"

# team selection
home_team_data <- get_cfb_team_data(cfb_preds, home_team_input)
away_team_data <- get_cfb_team_data(cfb_preds, away_team_input)

# predicting winner
winner_info <- predict_cfb_winner(cfb_preds, home_team_input, away_team_input, season = c(2023, 2024), predictor = xgb_model_cfb)

# score projections
predicted_scores <- predict_cfb_scores(cfb_points_pred, home_team_input, away_team_input, season = c(2023, 2024), home_score_model_cfb, away_score_model_cfb)
...
# results
cat(sprintf("ProphetML predicts %s will win with a %.2f%% probability, scoring %0.f points, while %s will score %0.f points.\n The total score for %s versus %s will be %.0f points, with %s winning by %.0f points.\n", winner_info$winner, winner_info$win_probability * 100, original_home_score, winner_info$loser, original_away_score, home_team_input, away_team_input, original_home_score + original_away_score, winner_info$winner, abs(original_home_score - original_away_score)))
